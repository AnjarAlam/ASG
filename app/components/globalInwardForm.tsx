...
export default function LocalInwardForm({ inwardToken }: LocalInwardFormProps) {
  ...
  const { createInward, loading: storeLoading } = useInwardStore();

  const fileInputRef = useRef<HTMLInputElement>(null);
  const vehicleInputRef = useRef<HTMLInputElement>(null);

  const [formData, setFormData] = useState({
    // Token is now passed from parent (InwardEntryPage)
  ...
  // Update token if parent changes it (shouldn't normally happen)
  useEffect(() => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      images: [...prev.images, ...selected.slice(0, remaining)],
    }));
  };
  ...
  const validateForm = () => {
    ...
    if (Number(formData.tareWeight) < 0)
      newErrors.tareWeight = "Tare cannot be negative";
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black text-gray-100">
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          {/* ── Token Display (passed from parent) ───────────────────────────
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <section className="bg-gray-900/75 border border-gray-800/70 rounded-2xl p-6 lg:p-8 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 mb-7 pb-4 border-b border-gray-700/50">
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="inline-flex rounded-full bg-gray-800/70 p-1 border border-gray-700/60 shadow-inner">
                    <button
                      type="button"
                      onClick={() =>
                        setFormData((p) => ({ ...p, supplier: "COLLIARY" }))
                      }
                      className={`
        ...
        `}
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setFormData((p) => ({ ...p, supplier: "PARTY" }))
                      }
                      className={`
          px-5 py-1.5 text-sm font-medium rounded-full transition-all duration-300
          ${
  ...
  <div className="space-y-6">
    ...
    <div className="relative">
      ...
      {errors.coalSize && (
        <p className="text-red-400 text-xs mt-1.5">{errors.coalSize}</p>
      )}
    </div>
    ...
    <div className="relative">
      <input
        type="datetime-local"
        name="inwardDateTime"
        value={formData.inwardDateTime.slice(0, 16)}
        onChange={(e) =>
          setFormData((p) => ({
            ...p,
